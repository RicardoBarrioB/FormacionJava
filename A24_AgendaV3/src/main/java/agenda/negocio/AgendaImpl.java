...
public class AgendaImpl implements Agenda{
	...
	//	cDao = new ContactoDaoJPA();   Con Spring
	}
	...
	public Contacto eliminar(int idContacto) {
		Contacto con = buscar(idContacto);
		eliminar(con);
		return con;
	}
	...
	private Comparator<Contacto> getCompApodo(){
		...
		return (o1,o2) -> col.compare(o1.getApodo() + o1.getIdContacto(), o2.getApodo() + o2.getIdContacto());
	...
	public int importarCSV(String fichero) throws IOException {
		...
		for(String[] array : datos) {
			...
			try {
				...
				//gardamos igual el contacto con numero = 0
				//hacer algun tipo de log
			}
			try {
				con.getDom().setPiso(Integer.parseInt(array[6]));
			}catch(NumberFormatException e) {
				//gardamos igual el contacto con piso = 0
				//hacer algun tipo de log
			}
			...
			Arrays.stream(array[11].split("/")).forEach(con::addTelefono);
			Arrays.stream(array[12].split("/")).forEach(con::addCorreo);
	...
	private List<String[]> importar(String fichero){
		...
		try (BufferedReader br = new BufferedReader(new FileReader(fichero))) {
			//String linea;
				...
				br.lines().map(x -> x.split(";")).forEach(datos::add);
